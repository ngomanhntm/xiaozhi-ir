...
def process_custom_emoji_collection(emoji_collection_dir, assets_dir, prefix, target_size=None):
    """Process a custom emoji collection directory, prefixing filenames and resizing if needed"""
    if not emoji_collection_dir:
        return []
    
    if not os.path.exists(emoji_collection_dir):
        print(f"Warning: Custom emoji collection directory not found: {emoji_collection_dir}")
        return []
    ...
    for root, dirs, files in os.walk(emoji_collection_dir):
        for file in files:
            if file.lower().endswith(('.png', '.gif')):
                ...
                dst_file = os.path.join(assets_dir, dst_filename)
                
                success = False
                if target_size and file.lower().endswith('.png'):
                    try:
                        from PIL import Image
                        with Image.open(src_file) as img:
                            if img.size != target_size:
                                print(f"Resizing {file} from {img.size} to {target_size}...")
                                # Resize image
                        ...
                        print(f"Warning: Failed to resize {file}: {e}, falling back to copy")
                        success = copy_file(src_file, dst_file)
                elif target_size and file.lower().endswith('.gif'):
                    try:
                        from PIL import Image, ImageSequence
                        with Image.open(src_file) as img:
                            # Check if we need to resize or skip frames
                            frames_count = 0
                            for _ in ImageSequence.Iterator(img):
                                frames_count += 1
                                
                            needs_resize = img.size != target_size
                            needs_skip = frames_count > 50 or os.path.getsize(src_file) > 1024 * 1024
                            
                            if needs_resize or needs_skip:
                                print(f"Processing GIF {file} (resize={needs_resize}, skip={needs_skip})...")
                                frames = []
                                duration = img.info.get('duration', 100)
                                skip_factor = 2 if needs_skip else 1
                                ...
                                frames[0].save(dst_file, save_all=True, append_images=frames[1:], 
                                               loop=img.info.get('loop', 0), duration=duration * skip_factor, disposal=2)
                                success = True
                            else:
                                success = copy_file(src_file, dst_file)
                    except Exception as e:
                        print(f"Warning: Failed to process GIF {file}: {e}, falling back to copy")
                        success = copy_file(src_file, dst_file)
                else:
                    success = copy_file(src_file, dst_file)
...
def generate_index_json(assets_dir, srmodels, text_font, emoji_collection, extra_files=None, multinet_model_info=None, nezuko_emoji_collection=None, con_ech_emoji_collection=None, trex_emoji_collection=None, xe_cuu_hoa_emoji_collection=None):
    ...
    if nezuko_emoji_collection:
        index_data["nezuko_emoji_collection"] = nezuko_emoji_collection
        
    if con_ech_emoji_collection:
        index_data["con_ech_emoji_collection"] = con_ech_emoji_collection

    if trex_emoji_collection:
        index_data["trex_emoji_collection"] = trex_emoji_collection

    if xe_cuu_hoa_emoji_collection:
        index_data["xe_cuu_hoa_emoji_collection"] = xe_cuu_hoa_emoji_collection
...
def build_assets_integrated(wakenet_model_paths, multinet_model_paths, text_font_path, emoji_collection_path, extra_files_path, output_path, multinet_model_info=None, nezuko_emoji_collection_path=None, conech_emoji_collection_path=None, trex_emoji_collection_path=None, xe_cuu_hoa_emoji_collection_path=None):
    """
    ...
    try:
        ...
        # Detect size of default emojis to match resolution
        target_size = (200, 200)
        if emoji_collection_path and os.path.exists(emoji_collection_path):
            for f in os.listdir(emoji_collection_path):
                if f.lower().endswith('.png'):
                    try:
                        from PIL import Image
                        with Image.open(os.path.join(emoji_collection_path, f)) as img:
                            target_size = img.size
                            print(f"Detected default emoji size: {target_size}")
                            break
                    except Exception as e:
                        print(f"Warning: Failed to detect default emoji size: {e}")
                        
        nezuko_emoji_collection = None
        if nezuko_emoji_collection_path:
            nezuko_emoji_collection = process_custom_emoji_collection(nezuko_emoji_collection_path, assets_dir, "nezuko", target_size)
            
        con_ech_emoji_collection = None
        if conech_emoji_collection_path:
            con_ech_emoji_collection = process_custom_emoji_collection(conech_emoji_collection_path, assets_dir, "con_ech", target_size)
            
        trex_emoji_collection = None
        if trex_emoji_collection_path:
            trex_emoji_collection = process_custom_emoji_collection(trex_emoji_collection_path, assets_dir, "trex", target_size)

        xe_cuu_hoa_emoji_collection = None
        if xe_cuu_hoa_emoji_collection_path:
            xe_cuu_hoa_emoji_collection = process_custom_emoji_collection(xe_cuu_hoa_emoji_collection_path, assets_dir, "xe_cuu_hoa", target_size)
            
        extra_files = process_extra_files(extra_files_path, assets_dir) if extra_files_path else None
        
        # Generate index.json
        generate_index_json(assets_dir, srmodels, text_font, emoji_collection, extra_files, multinet_model_info, nezuko_emoji_collection, con_ech_emoji_collection, trex_emoji_collection, xe_cuu_hoa_emoji_collection)
...
def main():
    ...
    parser.add_argument('--nezuko_emoji_collection', help='Path to Nezuko emoji collection directory')
    parser.add_argument('--conech_emoji_collection', help='Path to Conech emoji collection directory')
    parser.add_argument('--trex_emoji_collection', help='Path to Trex emoji collection directory')
    parser.add_argument('--xe_cuu_hoa_emoji_collection', help='Path to Xe_cuu_hoa emoji collection directory')
    ...
    if not wakenet_model_paths and not multinet_model_paths and not text_font_path and not emoji_collection_path and not extra_files_path and not multinet_model_info and not args.nezuko_emoji_collection and not args.conech_emoji_collection and not args.trex_emoji_collection and not args.xe_cuu_hoa_emoji_collection:
        print("Warning: No assets to build (no SR models, text font, emoji collection, extra files, custom wake word, or custom emojis)")
    ...
    success = build_assets_integrated(wakenet_model_paths, multinet_model_paths, text_font_path, emoji_collection_path, 
                                     extra_files_path, args.output, multinet_model_info, args.nezuko_emoji_collection, args.conech_emoji_collection, args.trex_emoji_collection, args.xe_cuu_hoa_emoji_collection)
