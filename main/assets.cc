...
bool Assets::LvglStrategy::Apply(Assets* assets, bool refresh_display_theme) {
    ...
    auto nezuko_theme = theme_manager.GetTheme("nezuko");
    auto con_ech_theme = theme_manager.GetTheme("con_ech");
    auto trex_theme = theme_manager.GetTheme("trex");
    auto doremon_theme = theme_manager.GetTheme("doremon");
    auto xe_cuu_hoa_theme = theme_manager.GetTheme("xe_cuu_hoa");
    ...
    if (cJSON_IsArray(emoji_collection)) {
        ...
        if (nezuko_theme != nullptr && nezuko_theme->emoji_collection() == nullptr) {
            nezuko_theme->set_emoji_collection(custom_emoji_collection);
        }
        if (con_ech_theme != nullptr && con_ech_theme->emoji_collection() == nullptr) {
            con_ech_theme->set_emoji_collection(custom_emoji_collection);
        }
        if (trex_theme != nullptr && trex_theme->emoji_collection() == nullptr) {
            trex_theme->set_emoji_collection(custom_emoji_collection);
        }
        if (doremon_theme != nullptr && doremon_theme->emoji_collection() == nullptr) {
            doremon_theme->set_emoji_collection(custom_emoji_collection);
        }
        if (xe_cuu_hoa_theme != nullptr && xe_cuu_hoa_theme->emoji_collection() == nullptr) {
            xe_cuu_hoa_theme->set_emoji_collection(custom_emoji_collection);
        }
    }

    cJSON* nezuko_emoji_collection = cJSON_GetObjectItem(root, "nezuko_emoji_collection");
    if (cJSON_IsArray(nezuko_emoji_collection)) {
        auto custom_emoji_collection = std::make_shared<EmojiCollection>();
        int emoji_count = cJSON_GetArraySize(nezuko_emoji_collection);
        for (int i = 0; i < emoji_count; i++) {
            cJSON* emoji = cJSON_GetArrayItem(nezuko_emoji_collection, i);
            if (cJSON_IsObject(emoji)) {
                cJSON* name = cJSON_GetObjectItem(emoji, "name");
                cJSON* file = cJSON_GetObjectItem(emoji, "file");
                cJSON* eaf = cJSON_GetObjectItem(emoji, "eaf");
                if (cJSON_IsString(name) && cJSON_IsString(file) && (NULL == eaf)) {
                    if (!assets->GetAssetData(file->valuestring, ptr, size)) {
                        ESP_LOGE(TAG, "Nezuko Emoji %s image file %s is not found", name->valuestring, file->valuestring);
                        continue;
                    }
                    custom_emoji_collection->AddEmoji(name->valuestring, new LvglRawImage(ptr, size));
                }
    ...
    cJSON* con_ech_emoji_collection = cJSON_GetObjectItem(root, "con_ech_emoji_collection");
    if (cJSON_IsArray(con_ech_emoji_collection)) {
        auto custom_emoji_collection = std::make_shared<EmojiCollection>();
        int emoji_count = cJSON_GetArraySize(con_ech_emoji_collection);
        for (int i = 0; i < emoji_count; i++) {
            cJSON* emoji = cJSON_GetArrayItem(con_ech_emoji_collection, i);
            if (cJSON_IsObject(emoji)) {
                cJSON* name = cJSON_GetObjectItem(emoji, "name");
                cJSON* file = cJSON_GetObjectItem(emoji, "file");
                cJSON* eaf = cJSON_GetObjectItem(emoji, "eaf");
                if (cJSON_IsString(name) && cJSON_IsString(file) && (NULL == eaf)) {
                    if (!assets->GetAssetData(file->valuestring, ptr, size)) {
                        ESP_LOGE(TAG, "Con_ech Emoji %s image file %s is not found", name->valuestring, file->valuestring);
                        continue;
                    }
                    custom_emoji_collection->AddEmoji(name->valuestring, new LvglRawImage(ptr, size));
                }
    ...
    cJSON* trex_emoji_collection = cJSON_GetObjectItem(root, "trex_emoji_collection");
    if (cJSON_IsArray(trex_emoji_collection)) {
        auto custom_emoji_collection = std::make_shared<EmojiCollection>();
        int emoji_count = cJSON_GetArraySize(trex_emoji_collection);
        for (int i = 0; i < emoji_count; i++) {
            cJSON* emoji = cJSON_GetArrayItem(trex_emoji_collection, i);
            if (cJSON_IsObject(emoji)) {
                cJSON* name = cJSON_GetObjectItem(emoji, "name");
                cJSON* file = cJSON_GetObjectItem(emoji, "file");
                cJSON* eaf = cJSON_GetObjectItem(emoji, "eaf");
                if (cJSON_IsString(name) && cJSON_IsString(file) && (NULL == eaf)) {
                    if (!assets->GetAssetData(file->valuestring, ptr, size)) {
                        ESP_LOGE(TAG, "Trex Emoji %s image file %s is not found", name->valuestring, file->valuestring);
                        continue;
                    }
                    custom_emoji_collection->AddEmoji(name->valuestring, new LvglRawImage(ptr, size));
                }
    ...
    cJSON* xe_cuu_hoa_emoji_collection = cJSON_GetObjectItem(root, "xe_cuu_hoa_emoji_collection");
    if (cJSON_IsArray(xe_cuu_hoa_emoji_collection)) {
        auto custom_emoji_collection = std::make_shared<EmojiCollection>();
        int emoji_count = cJSON_GetArraySize(xe_cuu_hoa_emoji_collection);
        for (int i = 0; i < emoji_count; i++) {
            cJSON* emoji = cJSON_GetArrayItem(xe_cuu_hoa_emoji_collection, i);
            if (cJSON_IsObject(emoji)) {
                cJSON* name = cJSON_GetObjectItem(emoji, "name");
                cJSON* file = cJSON_GetObjectItem(emoji, "file");
                cJSON* eaf = cJSON_GetObjectItem(emoji, "eaf");
                if (cJSON_IsString(name) && cJSON_IsString(file) && (NULL == eaf)) {
                    if (!assets->GetAssetData(file->valuestring, ptr, size)) {
                        ESP_LOGE(TAG, "Xe_cuu_hoa Emoji %s image file %s is not found", name->valuestring, file->valuestring);
                        continue;
                    }
                    custom_emoji_collection->AddEmoji(name->valuestring, new LvglRawImage(ptr, size));
                }
    ...
    if (cJSON_IsObject(skin)) {
        ...
        if (cJSON_IsObject(dark_skin) && dark_theme != nullptr) {
            ...
            if (cJSON_IsString(text_color)) {
                ...
                if (nezuko_theme != nullptr) {
                    nezuko_theme->set_text_color(LvglTheme::ParseColor(text_color->valuestring));
                }
                if (con_ech_theme != nullptr) {
                    con_ech_theme->set_text_color(LvglTheme::ParseColor(text_color->valuestring));
                }
            }
            if (cJSON_IsString(background_color)) {
                ...
                if (nezuko_theme != nullptr) {
                    nezuko_theme->set_background_color(LvglTheme::ParseColor(background_color->valuestring));
                    nezuko_theme->set_chat_background_color(LvglTheme::ParseColor(background_color->valuestring));
                }
                if (con_ech_theme != nullptr) {
                    con_ech_theme->set_background_color(LvglTheme::ParseColor(background_color->valuestring));
                    con_ech_theme->set_chat_background_color(LvglTheme::ParseColor(background_color->valuestring));
                }
            }
            if (cJSON_IsString(background_image)) {
                if (!assets->GetAssetData(background_image->valuestring, ptr, size)) {
                ...
                if (nezuko_theme != nullptr) {
                    nezuko_theme->set_background_image(background_image);
                }
                if (con_ech_theme != nullptr) {
                    con_ech_theme->set_background_image(background_image);
                }
            }
        }
    }
