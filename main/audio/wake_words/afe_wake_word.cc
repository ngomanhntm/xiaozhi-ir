...
bool AfeWakeWord::Initialize(AudioCodec* codec, srmodel_list_t* models_list) {
    ...
    if (audio_detection_task_stack_ == nullptr) {
        audio_detection_task_stack_ = (StackType_t*)heap_caps_malloc(stack_size * sizeof(StackType_t), MALLOC_CAP_SPIRAM);
        assert(audio_detection_task_stack_ != nullptr);
    }
    if (audio_detection_task_buffer_ == nullptr) {
        audio_detection_task_buffer_ = (StaticTask_t*)heap_caps_malloc(sizeof(StaticTask_t), MALLOC_CAP_INTERNAL);
        assert(audio_detection_task_buffer_ != nullptr);
    }
    ...
    audio_detection_task_ = xTaskCreateStatic([](void* arg) {
        ...
        vTaskDelete(NULL);
    }, "audio_detection", stack_size, this, 3, audio_detection_task_stack_, audio_detection_task_buffer_);
