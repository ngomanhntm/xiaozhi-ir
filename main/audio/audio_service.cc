...
void AudioService::Start() {
    ...
    // 1. Allocate / reuse buffers for audio_input task
    uint32_t input_stack_size = 0;
#if CONFIG_USE_AUDIO_PROCESSOR
    input_stack_size = 2048 * 3;
#else
    input_stack_size = 2048 * 2;
#endif

    if (audio_input_task_stack_ == nullptr) {
        audio_input_task_stack_ = (StackType_t*)heap_caps_malloc(input_stack_size * sizeof(StackType_t), MALLOC_CAP_SPIRAM);
    ...
    // 2. Allocate / reuse buffers for audio_output task
    uint32_t output_stack_size = 0;
#if CONFIG_USE_AUDIO_PROCESSOR
    output_stack_size = 2048 * 2;
#else
    output_stack_size = 2048;
#endif

    if (audio_output_task_stack_ == nullptr) {
        audio_output_task_stack_ = (StackType_t*)heap_caps_malloc(output_stack_size * sizeof(StackType_t), MALLOC_CAP_SPIRAM);
    ...
    // 3. Allocate / reuse buffers for opus_codec task
    uint32_t codec_stack_size = 2048 * 12;
    if (opus_codec_task_stack_ == nullptr) {
        opus_codec_task_stack_ = (StackType_t*)heap_caps_malloc(codec_stack_size * sizeof(StackType_t), MALLOC_CAP_SPIRAM);
    ...
    audio_input_task_handle_ = xTaskCreateStaticPinnedToCore([](void* arg) {
        ...
        vTaskDelete(NULL);
    }, "audio_input", input_stack_size, this, 8, audio_input_task_stack_, audio_input_task_buffer_, 0);
    ...
    audio_output_task_handle_ = xTaskCreateStatic([](void* arg) {
        ...
        vTaskDelete(NULL);
    }, "audio_output", output_stack_size, this, 4, audio_output_task_stack_, audio_output_task_buffer_);
#else
    /* Start the audio input task */
    audio_input_task_handle_ = xTaskCreateStatic([](void* arg) {
        ...
        audio_service->AudioInputTask();
        vTaskDelete(NULL);
    }, "audio_input", input_stack_size, this, 8, audio_input_task_stack_, audio_input_task_buffer_);
    ...
    opus_codec_task_handle_ = xTaskCreateStatic([](void* arg) {
        ...
        vTaskDelete(NULL);
    }, "opus_codec", codec_stack_size, this, 2, opus_codec_task_stack_, opus_codec_task_buffer_);
}
