...
### PHẦN 3: THÊM BỘ BIỂU CẢM & CHỦ ĐỀ TÙY CHỈNH (EMOJI & THEME)

Để cá nhân hóa màn hình LCD hiển thị biểu cảm của robot cún theo chủ đề mới của riêng bạn (ví dụ: tạo bộ biểu cảm đặc biệt), thực hiện theo các bước sau:

#### Bước 1: Chuẩn bị hình ảnh biểu cảm
*   Chuẩn bị tối đa 21 tệp ảnh định dạng **PNG** tách nền (trong suốt) tương ứng với 21 trạng thái cảm xúc của Xiaozhi (như `happy.png`, `sad.png`, `angry.png`, `sleepy.png`...).
*   Ảnh sẽ tự động được script Python đóng gói resize về độ phân giải chuẩn **`200x200` pixels** để hiển thị tối ưu trên màn hình LCD.

#### Bước 2: Đăng ký Theme và nạp Emoji trong mã nguồn
1.  **Đăng ký Theme mới:** Mở tệp **[main/display/lcd_display.cc](file:///d:/project/xiaozhi-esp32-main/main/display/lcd_display.cc)**, định nghĩa màu sắc bong bóng chat cho theme mới trong hàm `InitializeLcdThemes()`:
    ```cpp
    auto my_theme = new LvglTheme("my_theme");
    my_theme->set_user_bubble_color(lv_color_hex(0x00FF00));      // Màu bong bóng người dùng
    my_theme->set_assistant_bubble_color(lv_color_hex(0x222222)); // Màu bong bóng AI
    LvglThemeManager::GetInstance().RegisterTheme("my_theme", my_theme);
    ```
2.  **Nạp Emoji từ file nhị phân:** Mở tệp **[main/assets.cc](file:///d:/project/xiaozhi-esp32-main/main/assets.cc)** để giải mã JSON chỉ mục và ánh xạ ảnh biểu cảm vào bộ nhớ đồ họa LVGL.

#### Bước 3: Đóng gói tài nguyên và nạp firmware
1.  Đăng ký thư mục ảnh mới trong script đóng gói **[scripts/build_default_assets.py](file:///d:/project/xiaozhi-esp32-main/scripts/build_default_assets.py)** và liên kết trong tệp **[main/CMakeLists.txt](file:///d:/project/xiaozhi-esp32-main/main/CMakeLists.txt)**.
2.  Xóa tệp đóng gói cũ và build/nạp lại firmware để cập nhật giao diện mới:
    ```powershell
    # Xóa file assets đóng gói cũ để bắt buộc rebuild
    Remove-Item -Path build/generated_assets.bin -ErrorAction SilentlyContinue
    
    # Biên dịch và nạp code lên ESP32
    idf.py build flash
    ```
3.  Ra lệnh giọng nói: *"Hãy chuyển sang giao diện my_theme"* để xem bộ biểu cảm độc quyền hiển thị trên màn hình robot!
